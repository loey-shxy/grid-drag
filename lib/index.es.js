import { defineComponent, ref, computed, resolveComponent, createElementBlock, openBlock, normalizeStyle, normalizeClass, renderSlot, createElementVNode, createVNode, createBlock, withCtx, createTextVNode, toDisplayString, createCommentVNode, Fragment, renderList, getCurrentInstance, inject, watch, onMounted, onBeforeUnmount, readonly, onUpdated, onUnmounted, TransitionGroup, isVNode, reactive, render, unref, createSlots, normalizeProps, guardReactiveProps, nextTick } from "vue";
const COLUMNS = 24;
function getComponentDistance(comp1, comp2) {
...
function shouldTriggerLayout(resizingComponent, newPosition, newSize, allComponents, gap) {
...
const validatePosition = (components, currentId, position, size, containerInfo, _gridConfig) => {
...
function findAvailablePosition(components, newComponent, containerInfo, gridConfig) {
  ...
  for (const comp of components) {
    const unitWidth2 = columnWidth + gap;
    const startCol = Math.floor(comp.x / unitWidth2);
...
function canAddComponent(components, newComponent, containerInfo, gridConfig) {
  const testComponents = [...components, { ...newComponent, x: 0, y: 0 }];
  return reorganizeLayout(testComponents, containerInfo, gridConfig);
}
function calculateAvailableSpace(components, containerInfo, _gridConfig) {
...
function snapToColumnGrid(position, containerInfo, gridConfig) {
...
function snapToColumnGridWithSize(position, componentSize, containerInfo, gridConfig) {
  const { width: containerWidth, height: containerHeight } = containerInfo;
  const { gap } = gridConfig;
  const columnWidth = (containerWidth - (COLUMNS - 1) * gap) / COLUMNS;
  const unitWidth = columnWidth + gap;
...
function autoFillComponentToGrid(component, columnWidth, gap) {
...
function resizeComponentWithAutoFill(component, newSize, gridConfig, containerWidth) {
  const { gap } = gridConfig;
  const columnWidth = containerWidth ? (containerWidth - (COLUMNS - 1) * gap) / COLUMNS : gridConfig.cellWidth;
  const minWidth = component.minWidth || columnWidth;
  const minHeight = component.minHeight || 60;
...
function getAffectedComponents(components, resizingComponent, newPosition, newSize, containerInfo, gridConfig) {
  const affected = [];
  ...
  const tempComponents = components.map(
    (comp) => comp.id === resizingComponent.id ? { ...comp, ...newPosition, ...newSize } : { ...comp }
  );
...
function validatePositionWithLayout(components, currentId, position, size, containerInfo, gridConfig) {
  const { width: containerWidth, height: containerHeight } = containerInfo;
  if (position.x < 0 || position.y < 0 || position.x + size.width > containerWidth || position.y + size.height > containerHeight) {
...
function validateDragPosition(components, currentId, newPosition, containerInfo, gridConfig) {
  ...
  const tempComponents = components.map(
    (comp) => comp.id === currentId ? draggedComponent : { ...comp }
  );
  const layoutSuccess = reorganizeLayout(tempComponents, containerInfo, gridConfig, true, false);
  if (!layoutSuccess) {
    return { valid: false, affectedComponents: [], finalPosition: snappedPosition };
...
const GridsterItem = /* @__PURE__ */ _export_sfc$1(_sfc_main$a, [["__scopeId", "data-v-cbc6ba49"]]);
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const filteredComponents = computed(() => {
      ...
      return props.componentLibrary.filter(
        (component) => component.name.toLowerCase().includes(query) || component.type.toLowerCase().includes(query)
      );
    });
...
const AddComponent = /* @__PURE__ */ _export_sfc$1(_sfc_main$9, [["__scopeId", "data-v-661f7b5e"]]);
          ...
          createElementVNode("div", {
            ...
            (openBlock(true), createElementBlock(Fragment, null, renderList(__props.addedComponents, (component) => {
              return openBlock(), createBlock(GridsterItem, {
                ...
                _: 2
              }, 1032, ["component", "grid-config", "container-info"]);
            }), 128))
          ], 36)
        ]),
...
const GridDrag = /* @__PURE__ */ _export_sfc$1(_sfc_main, [["__scopeId", "data-v-3cd88267"]]);
